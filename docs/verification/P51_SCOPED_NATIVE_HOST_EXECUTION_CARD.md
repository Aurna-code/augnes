...
| --- | --- |
| Application | `hynk-studio/augnes`, canonical checkout, branch `codex/p51-live-feasibility-card`, base `682578d3f2bdd4187948df1c427766747ec07f3d`. The retained manifest head has the same original tree. No historical checkout or reset is used. |
...
1. After PR review, retain the original artifacts and verify their hashes. Use
   the existing disposable lifecycle, database and real local operator-session
   admission. Prepare initial work and selected synthetic notes through their
   existing owners. Do not use the unauthenticated test option. Freeze the
...
startup markers remain absent. The actual Codex sandbox permits approved reads

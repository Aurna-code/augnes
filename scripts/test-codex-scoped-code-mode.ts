...
import { createCodexScopedTaskV01, prepareScopedCodexLaunchV01 } from "@/lib/vnext/native-host/codex-scoped-task";
...
async function runNativeFixture(parent: string, executable: string, scenario: "permissions" | "import" | "cancel" | "deadline", result: Json): Promise<void> {
  ...
  Object.assign(result, { scenario, scope: scope.fingerprint, configuration: launch.configuration_fingerprint,
    model_calls: 0, synthetic_responses: 0, command_events: [], tool_outputs: [], owned_pids: [] });
  ...
  try {
    ...
    responseServer.on("request", (req, res) => {
      ...
      req.on("end", () => { let phase = "method"; try {
        ...
        if (ordinal === 1) {
          ...
          events = [call("text({check:'symlink',result:await tools.exec_command({cmd:'/bin/cat TASK.txt',login:false,yield_time_ms:3000,max_output_tokens:200})});"), complete("fixture-2")];
        } else {
          if (scenario === "permissions") { rmSync(path.join(stage, "TASK.txt")); writeFileSync(path.join(stage, "TASK.txt"), text); }
    ...
    child = spawn(executable, args, { env: environment, cwd: stage, stdio: ["pipe", "pipe", "pipe"] });
    ...
    const thread = await request("thread/start", { cwd: stage, model: "gpt-6-astra", modelProvider: "mock_provider",
    ...
    const turn = await request("turn/start", { threadId, cwd: stage, permissions: launch.profile_name, approvalPolicy: "never",
    ...
    if (scenario === "permissions") {
      ...
      result.nested_checks = Object.fromEntries(["allowed", "held", "evaluator", "outside", "write", "network", "environment", "symlink"].map(check => [check, find(check).result.exit_code]));
      assert.equal(find("allowed").result.output, text.repeat(3)); assert.equal(find("allowed").result.exit_code, 0);
      for (const check of ["held", "evaluator", "outside", "write", "network"]) assert.notEqual(find(check).result.exit_code, 0, `nested_denial:${check}`);
      ...
      assert.notEqual(find("symlink").result.exit_code, 0, "selected_path_symlink_replacement_must_not_widen_read_grant");
      ...
      for (const filename of files) assert.equal(readFileSync(path.join(stage, filename.relative_path), "utf8"), text);

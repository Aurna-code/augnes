...
import { admitPersistedHostTaskContextPacketV01 } from "./direct-native-host-round-trip";
...
import { createCodexScopedTaskV01 } from "@/lib/vnext/native-host/codex-scoped-task";
...
function parseRequest(value: unknown): DefineAuthoredSuccessorTaskRequestV01 {
  ...
  check(equal(Object.keys(r).sort(), ["action", "definition", "expected_active_selection_revision", "expected_current_packet_fingerprint",
    "expected_current_packet_id", "expected_latest_receipt_fingerprint", "expected_latest_receipt_id", "expected_root_fingerprint"]), "request_fields");
  ...
  return { ...r, definition: normalizeAuthoredSuccessorTaskV01(r.definition) };
}
...
function source(db: Database.Database, config: VNextLocalOperatorPilotConfigV01, request: DefineAuthoredSuccessorTaskRequestV01) {
  ...
  check(r.packet && r.receipt.integrity.fingerprint === request.expected_latest_receipt_fingerprint &&
    ...
    (!r.run || (r.run.status === "completed" && r.run.metadata.reconciliation_required === false &&
      r.run.metadata.terminal_receipt_persisted === true)) && r.receipt.execution.status === "completed" &&
...
function build(input: { prior: TaskContextPacketV01; receipt: ReturnType<typeof source>["receipt"];
  ...
  const entries = [
    ...
    { entry_id: `successor-predecessor:${receipt.receipt_id}`, entry_kind: "evidence_ref" as const, source_ref: receipt.integrity.fingerprint, external_ref: receiptRef,
      ...
      bounded_summary: `Native execution: ${receipt.execution.status}; verification: ${receipt.verification.status}. No proposal acceptance is implied.`,
  ...
  const packet = buildTaskContextPacketV01({ workspace_id: prior.workspace_id, project_id: prior.project_id, work_ref: definitionRef,
    generated_at: at, expires_at: prior.expires_at,
    ...
    compatibility: { source_contracts: [AUTHORED_SUCCESSOR_TASK_V01], legacy_scope_ref: null,
...
export async function defineAuthoredSuccessorTaskV01(db: Database.Database, input: {
  ...
  const admission = await admitPersistedHostTaskContextPacketV01(db, { config: input.config,
    packet_id: request.expected_current_packet_id, packet_fingerprint: request.expected_current_packet_fingerprint,
    evaluated_at: input.clock?.now() ?? new Date().toISOString(), require_active_project: true });
  check(admission.root_scope.root_fingerprint === request.expected_root_fingerprint, "root_changed");
  ...
  try {
    ...
    const built = build({ prior: predecessor.packet, receipt: predecessor.receipt, operator_id: input.config.operator_id,
      at: auth.action_observed_at, material: { request, source_root_ref: admission.root_scope.root_scope_ref, physical_root_fingerprint: fingerprintNativeHostPhysicalRootIdentityV01(admission.root_scope.physical_root_identity), session_id: auth.session.session_id } });
...
export async function prepareAuthoredSuccessorHandoffV01(db: Database.Database, input: {
  ...
  const admission = await admitPersistedHostTaskContextPacketV01(db, { ...input, evaluated_at: new Date().toISOString(), require_active_project: true });

...
import { SELECTED_WORK_SOURCE_LABELS, type SelectedWorkSourceInput, type SelectedWorkSourceSelection } from "@/types/vnext/project-work-revision";
...
export function SelectedWorkSourceEditor({ initialization, busy, onChange }: {
  ...
  const [notes, setNotes] = useState<SelectedWorkSourceInput[]>(() => (initialization.selected_source_context ?? []).map((entry) => ({
  ...
  function changeNotes(next: SelectedWorkSourceInput[]) {
    revision.current += 1;
    setNotes(next);
    setComparison(null);
    setError(null);
    ...
    try {
      const response = await fetch("/api/vnext/operator/project-continuity", {
        ...
        body: JSON.stringify({ action: "compare_selected_work_sources",
          ...
          expected_current_packet_fingerprint: packet.packet_fingerprint, notes }),
      });
      ...
      onChange({ selected_source_context: body.comparison.entries, expected_source_comparison: body.comparison.fingerprint }, false);
    } catch (failure) {
  ...
  return <details className={styles.panel} data-selected-work-sources>
    ...
    {notes.map((note, index) => <div key={index} className={styles.panel}>
      ...
      <button type="button" className={styles.secondaryButton} disabled={busy || comparing}
